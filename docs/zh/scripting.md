...
| 别名（地址别名） | `let p = &buf[0];`（通过 `&expr` 或 `&expr + 常量` 创建） | 给“DWARF 地址表达式”（指针/数组/成员地址）起一个脚本名。可把复杂的 DWARF 变量类型/访问表达式设为别名，便于在脚本中复用。 | 仅作为“地址”使用：可传给 `memcmp/strncmp/starts_with`，或用于 `{:x.N}`/`{:s.N}`/`{:p}` 等按地址读取/打印的场景；不是通用指针：不支持解引用；除绑定时常量偏移外不参与算术；不用于通用比较。 |

说明：
1. 目前脚本层不直接暴露结构体/数组/指针类型；对于这些聚合类型，请通过 DWARF 变量访问（成员访问、解引用、常量下标）先得到标量再参与运算。唯一的例外是“地址别名”变量（`&expr` 或 `&expr + 常量`），它仅作为“可命名的地址”用于内存类操作和指针/字节格式化。
