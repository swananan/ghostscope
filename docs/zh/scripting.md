...
地址；同一模块地址的别名只会 attach 一次。候选函数名同时来自 DWARF/GDB 函数
索引和 ELF 文本符号，因此只有符号表、没有 DWARF 的模块也支持前缀匹配。
...
- C 兼容字节串：`CString`，以及布局验证通过的 `&CStr` 和 `Box<CStr>`；
...
显式提供 data pointer 和 length 元数据。更老的布局保留原生 DWARF 展示。
