...
| 源语言 | 支持程度 | 当前现状 |
| --- | --- | --- |
| C | 最好 | 这是当前的主要优化目标。局部变量、全局变量、指针、数组、结构体、枚举和 C 字符串这类 C 风格布局，与现有 DWARF 读取和脚本运算最匹配。 |
...
- 对 C++ 和 Rust，建议把 GhostScope 理解为“理解 DWARF 布局”，而不是“理解语言语义”。
...
| 别名（DWARF 表达式别名） | `let a = global.arr;`、`let p = &buf[0];` | 给任意 DWARF 表达式（变量、成员访问、数组、指针解引用或取地址）起一个脚本名，便于复用复杂类型/路径。 | 与底层 DWARF 类型具备相同的“复杂访问”能力：成员访问（`a.field`）、整数字面量下标（`a[0]`）、取地址（`&a`），并可用于 `memcmp/strncmp/starts_with` 以及 `{:x.N}`/`{:s.N}`/`{:p}`。指针算术仍仅限“加非负常量”。 |

说明：
1. 目前脚本层不直接暴露结构体/数组/指针类型；对于这些聚合类型，请通过 DWARF 变量访问（成员访问、解引用、整数字面量下标）先得到标量再参与运算。例外是“别名”变量：它可以绑定到任意 DWARF 表达式，并作为可复用的“基表达式”用于成员/下标访问、取地址与内存格式化。
...
| 数组 | `T[n]` | Array | 支持整数字面量下标读取（顶层或链尾）；暂不支持动态/中间索引、多维数组 |
