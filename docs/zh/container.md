...
#### 场景 6：GhostScope 在一个 private PID namespace 容器里，被追踪进程不在这个 PID namespace 里
...
这一步先把明显不成立的情况排除掉，也把场景 6 这类“目标根本不在当前 `/proc` 视角里”的情况尽早挡住。
...
当前容器 e2e 主要覆盖的是 GhostScope 与目标进程运行在同一容器 PID 视角里的 smoke 场景；它还没有真正覆盖“GhostScope 在 host、目标在 private PID namespace 容器里”的 `-t` 生命周期维护问题。
