...
| 6. private PID namespace 容器 -> 当前容器可见的更内层 / 子层 private PID namespace | 在当前容器 shell 中输入、且是当前容器 `/proc` 视角可见的 PID，不是子容器里看到的局部 PID | 在当前容器 `/proc` 视角下，通常等于 `input_pid` | 通常不同于 `input_pid` 和 `proc_pid`；对应 `NSpid` 第一项 | 通常不同于 `input_pid` 和 `proc_pid`；对应 `NSpid` 链条尾部，往往就是子容器内看到的 PID | helper 可用：`bpf_get_ns_current_pid_tgid(...).tgid == container_pid`；helper 不可用：只有在 `NSpid` 给出明确 host 映射时才回退到 `bpf_get_current_pid_tgid() >> 32 == host_pid`，否则直接 fail fast | `bpf_get_current_pid_tgid() >> 32`，通常仍对齐 `host_pid` | helper 可用时通常先拿到 `container_pid`；helper 不可用时通常先拿到 `host_pid`；无论哪种，最终都还要归一回外层容器可见的 `proc_pid` | 外层容器 `/proc` 视角下的 `proc_pid`，不是子容器本地 `container_pid` | 条件支持 | 条件支持 |
...
- 场景 6 对 PID 视角要求最严格：`-p` 和 backtrace `-t` 现在都有 nested 验证覆盖，但 `globals_target` nested `-t` 用例仍会显式跳过，并等待一条能把运行时 PID 稳定 alias 回外层容器 `proc_pid` 的路径。
...
- `fork`
- `exit`
- 启用运行时模块刷新时的 map-change 事件（`mmap`、`mprotect`、`munmap`、`mremap`）

当前实现里，独立 `-t` 默认启动这条链路，除非配置中设置了 `enable_sysmon_for_target = false`。`-p` 模式使用更窄的 sysmon 路径监控指定 PID 的 map-change。组合使用 `-t <path> -p <pid>` 时使用 `-p` 提供的进程上下文，而不是 target-mode sysmon：`-t` 只负责把函数/源码行/地址目标解析限定到一个模块，`-p` 提供具体进程映射和 PID 过滤。`sysmon` 主要服务于独立 `-t` 模式，尤其是需要在目标进程启动后持续维护模块 offsets、运行时 backtrace 模块、allowlist 和退出清理的时候。
...
- 在“外层容器 -> 子容器”拓扑下，backtrace `-t` 现在已经进入受支持的 container-e2e 覆盖，而不是继续跳过。
- 难点仍然是结构性的：`sysmon` 可能先学到 host 视角的 `event_pid`，也可能先看到最内层 namespace 的 TGID；而 `proc_module_offsets` 仍然按外层容器当前 userspace `/proc` 视角里的 `proc_pid` 做 key，因为这张 map 本来就是按 GhostScope 当前环境里的 `/proc/<pid>/maps` 填出来的。
- 当前实现会为解析出的运行时 PID 候选写入 offsets，并维护从运行时 lookup PID 回到外层容器 `proc_pid` 的 `pid_aliases`。
- 因此这仍是部分 target-mode 覆盖：backtrace 用例会验证这条 alias 路径，但 `globals_target` nested `-t` 用例仍会跳过，直到全局变量路径也具备同样稳定的映射保证。

当前容器 e2e 已经覆盖 host-to-container、same-container 和 outer-container-to-child-container 这些 backtrace `-t` 路径。nested child-container 仍是生命周期维护风险最高的一类，所以继续保留在 full container-e2e CI 矩阵中。
...
- 从当前容器仍可见的更内层 / 子层 PID namespace，仍然属于预期范围，而且 `-p` 和 backtrace `-t` 现在都有“外层容器 -> 子容器目标”的专门验证路径，并进入 full container-e2e CI 矩阵。
- nested child-container 的 `-t` 仍然属于部分覆盖：runtime event 可能暴露 host 视角或最内层 namespace 的 PID，而 `proc_module_offsets` 仍按外层容器 `/proc` 的 PID 视角建 key；backtrace 用例通过解析出的运行时 PID 候选和定向的 `pid_aliases` 来桥接这条关系，而 `globals_target` nested `-t` 用例仍会跳过。
...
- `-t` 模式依赖 `sysmon` 维护运行时进程生命周期和模块映射状态；而 `sysmon` 的 `event_pid` 来自 `bpf_get_current_pid_tgid() >> 32`，对齐的是 host 视角 PID。
- 在跨 PID namespace 场景里，`-t` 需要把 `event_pid`、`proc_pid` 和 `proc_module_offsets` 持续对齐到足够稳定，才能正确完成 `/proc` 读取、offset 写入、运行时模块刷新和退出清理；这一点目前仍是结构性薄弱点。
- 其中，nested child-container 的 `-t` 在 e2e 中仍是部分覆盖。backtrace 用例会运行，但 `globals_target` 用例仍会跳过，因为它们需要稳定维护“运行时 / event PID 候选 -> 外层容器 `proc_pid`”的映射；`proc_module_offsets` 仍然是按外层 `/proc/<pid>/maps` 这套视角来做全局变量和共享库重定位的。
