...
当前容器 e2e 已经覆盖了当前被视为支持的 host-to-container 和 same-container `-t` 路径。“外层容器 -> 更内层 / 子层 PID namespace 目标”这个拓扑现在也重新进入了 full container-e2e CI 矩阵，用来持续验证当前被视为支持的覆盖面；与此同时，nested child-container 的 `-t` 用例仍会带注释地显式跳过，不再依赖运行时 alias 猜测。
...
- 从当前容器仍可见的更内层 / 子层 PID namespace，仍然属于预期范围，而且 `-p` 现在已经有“外层容器 -> 子容器目标”的专门验证路径；这个拓扑现在已经回到 full container-e2e CI 矩阵，用来持续验证当前被视为支持的覆盖面。nested `-t` 的生命周期维护仍然是另一条限制链路。
