...
静态展示限制与运行时读取状态分开传递。DWARF 层返回 `ValueReadPlanResolution`，
分别保存可选的采集计划、带路径的静态提示和根 adapter 拒绝报告。没有语义采集
计划时仍保留提示，普通 DWARF 读取继续使用原有运行时地址检查。编译器补充
实际嵌套采集预算造成的降级，对内存值和寄存器值统一完成诊断关联，绑定到
`TraceContext` 内的表达式和类型索引。多个内部索引对应同一用户
表达式时，设置提示按解析后的表达式、类型和原因去重。

CLI、TUI 创建结果和 trace 快照消费相同的结构化诊断。快照保留原因，因此
`info trace` 不依赖日志或 loader actor 的生命周期。运行时读取失败继续使用
`VariableStatus` 和每个嵌套子值自己的状态；格式化器不解析日志来推断失败。
诊断元数据不增加 eBPF 内存读取，也不改变事件的二进制状态布局。新增的
序列宽度元数据用于区分确实达到元素上限的截断，旧元数据缺失时保留通用原因。

语义深度边界上的说明使用有界的类型信息检查，避免把纯字段 struct 错报为
语义展示失败；它不展开目标的运行时指针图。枚举路径的静态提示只说明可能
分支的展示限制，不能用于断言该分支发生过读取错误。用户提示与处理建议见
[值诊断](value-diagnostics.md)。

CLI 将同一份英文 Markdown 嵌入 `--value-diagnostics-help`，沿用 `--script-help` 的
提前退出路径。CI 检查两个命令的输出与源文档一致。Skill 引导模型读取
已安装二进制中的说明，不单独维护诊断指南副本。
