...
- 函数名通配符目标（`trace <function_prefix*> { ... }`）：
  ...
  - 匹配 DWARF/GDB 函数索引和 ELF 文本符号中的可挂载名称。
...
- `模块后缀:0xADDR`：指定模块 + 地址。模块部分支持“全路径”或“唯一后缀”匹配；若后缀匹配不唯一，会报歧义并列出候选。在 `-t -p` 会话中，该模块必须匹配 `-t` 目标。
- `[verbose|v]`：可选。显示 DWARF 位置表达式（默认隐藏）。

地址参数不是原始 ELF 文件偏移，也不是 `/proc/<pid>/maps` 中 ASLR 后的运行时地址。GhostScope 会在内部把模块虚拟地址转换为 uprobe 需要的文件偏移。

**默认与模式：**
- 使用 `-t <binary>`（目标文件模式）启动：默认模块为 `<binary>`。
- 使用 `-p <pid>`（PID 模式）启动：默认模块为进程的主可执行文件。若地址来自共享库，请通过模块后缀显式指定，或使用 `-t` 配合该 `.so`。
- 同时使用 `-t <binary>` 和 `-p <pid>` 启动：默认模块为 `<binary>`，`-p` 只把运行时事件限制到该 PID。
...
**说明：**
- `ui source off`：隐藏源码面板，只保留 eBPF 输出 与 命令交互 两个面板。适用于没有源码的环境。
- `ui source on`：重新显示源码面板。开启后会立即请求加载源码。

**提示：** 也可通过命令行或配置控制：
- 命令行：`--no-source-panel`（隐藏）/ `--source-panel`（显示）
- 配置：`[ui].show_source_panel=false`

**布局比例：**
- 隐藏源码面板时，两面板比例遵循 `[ui].two_panel_ratios`；若未配置，则回退为 `[ui].panel_ratios` 的后两项（例：`4:3:3` → `3:3`）。
