...
impl DebugSession {
    ...
    pub fn new(args: &ParsedArgs) -> Result<Self> {
        ...
        let binary_analyzer = if let Some(pid) = args.pid {
            ...
            match BinaryAnalyzer::from_pid(pid, debug_path) {
                ...
                Err(e) => {
                    warn!("Failed to analyze binary for PID {}: {}", pid, e);
                    warn!("Continuing without binary analysis...");
                    None
                }
            }
            ...
            match BinaryAnalyzer::new(binary_path, debug_path) {
                ...
                Err(e) => {
                    warn!("Failed to analyze binary '{}': {}", binary_path, e);
                    warn!("Continuing without binary analysis...");
                    None
                }
            }
    ...
    pub async fn initialize_ebpf(&mut self, bytecode: &[u8]) -> Result<()> {
        info!("Initializing eBPF loader with {} bytes of bytecode", bytecode.len());
        
        let loader = GhostScopeLoader::new(bytecode)
    ...
    pub async fn attach_uprobes(&mut self, uprobe_configs: &[ghostscope_compiler::UProbeConfig]) -> Result<()> {
        info!("Attaching {} uprobes", uprobe_configs.len());
        
        for (i, config) in uprobe_configs.iter().enumerate() {
            info!("Attaching uprobe {}: {:?}", i, config.function_name.as_ref().unwrap_or(&"<address>".to_string()));
    ...
    async fn attach_single_uprobe(&mut self, config: &ghostscope_compiler::UProbeConfig) -> Result<()> {
        // Create a new loader for this uprobe configuration
        info!("    Creating new eBPF loader with {} bytes of bytecode", config.ebpf_bytecode.len());
        ...
        if let Some(uprobe_offset) = config.uprobe_offset {
            if let Some(ref function_name) = config.function_name {
                ...
                info!("    Attaching to address 0x{:x} in {} using eBPF function '{}'", 
    ...
    pub async fn attach_uprobe(&mut self) -> Result<()> {
        ...
        loader.attach_uprobe(&binary_path, function_name, function_offset, self.target_pid.map(|pid| pid as i32))
    ...
    pub async fn start_monitoring(&mut self) -> Result<()> {
        ...
        if !self.loaders.is_empty() {
            info!("Starting event monitoring loop for {} loaders", self.loaders.len());
            ...
            return Err(anyhow::anyhow!("No eBPF loaders initialized - unable to monitor events"));
        }
    ...
    pub fn list_functions(&self) -> Vec<String> {
        if let Some(ref analyzer) = self.binary_analyzer {
            analyzer.symbol_table.get_functions()
                .iter()
    ...
    pub fn find_functions(&self, pattern: &str) -> Vec<String> {
        if let Some(ref analyzer) = self.binary_analyzer {
            analyzer.symbol_table.find_matching(pattern)
                .iter()
                .map(|sym| sym.name.clone())
                .collect()
        } else {
